...
class ChatConsumer(AsyncConsumer):
    ...
    def create_message(self, text):
        Message.objects.create(chat_id=self.chat.id, 
...
class VideoChatConsumer(AsyncConsumer):
    ...
    def create_videothread(self, callee_username):
        ...
        if VideoThread.objects.filter(
            Q(caller_id=callee.id) | Q(callee_id=callee.id), status=VC_PROCESSING).count() > 0:
            return VC_BUSY, None
    ...
    def change_videothread_datetime(self, id, is_start):
        try:
            videothread = VideoThread.objects.get(id=id)
            if is_start:
                videothread.started_at = datetime.now()
            else:
                videothread.ended_at = datetime.now()
            videothread.save()
            return videothread
        except VideoThread.DoesNotExist:
            return None